...
val_dataloader = DataLoader(
    ImageDataset("../../data/%s" % opt.dataset_name, transforms_=transforms_, mode="test", unaligned=True),
    batch_size=16,
    shuffle=True,
    num_workers=opt.n_cpu,
)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            cost_pixelwise.append(loss_pixelwise.item())
            cost_cycle.append(loss_cycle.item())
